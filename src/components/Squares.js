...
export default function Squares() {
  const [squares, setSquares] = useState(["sqA", "sqB", "sqC", "sqD"]);
  const [activeSquare, setActiveSquare] = useState(null); // Use the state hook twice, as we need two slices of state: 'squares' and
  ...
  const getClassName = (id) => {
    let isActive = activeSquare === id ? " active" : "";
    return isActive;
  };
